...
from ..forms import Subscribe,actuelle,collab, Free,IdeeForm, UpdateProjetForm, CommentForm_capteurs, StudentSignUpForm,ProfileCUpdateForm, CapteurForm, EventForm,ProjetForm, EquipeForm, UserUpdateForm, ProfileUpdateForm ,TypeCapteurForm, DomaineForm, SubjectForm, StudentInterestsForm, CommentForm
from ..models import Idee, Capteur,Question, Answer,Idee, Chaine,Collaboration_connaissances, Comment_display, Collaboration_actuelle,Equipe,favoris_expert, Quiz,Forum, Student,ExpertListe, TakenQuiz, User, Subject, Profile,Comment, Domaine, TypeCapteur
...
def index(request):
    context = {}
    context['questions'] = Idee.objects.all()
    return render(request, 'boite/index.html', context)
...
@login_required
def BoiteListView(request):
    context = {}
    context['idees'] = Idee.objects.all()
    return render(request, 'boite/idee.html', context)
...
@method_decorator([login_required, student_required], name='dispatch')
class IdeeCreateView(CreateView):
    model = Idee
    form_class = IdeeForm

    template_name = 'boite/idee_form.html'
    ...
    def form_valid(self, form):
        ...
        quiz.save()
        form.save_m2m()
        return redirect('students:boite_list')
